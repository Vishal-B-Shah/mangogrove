...
const db = new sqlite3.Database(path.join(__dirname, 'mangoes.db'), (err) => {
  if (err) { console.error('DB error:', err.message); process.exit(1); }
  console.log('📦 Database ready (mangoes.db)');
});

// Helper: run a query that returns nothing (INSERT / UPDATE / DELETE / CREATE)
...
async function initDB() {
  ...
  await run(`CREATE TABLE IF NOT EXISTS stock_log (
    id         INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  if (c === 0) {
    ...
    const seeds = [
      ['Alphonso (Hapus)', '🥭', 599, 80,  'Ratnagiri, Maharashtra', 'The king of mangoes. Buttery, fibre-free, intensely sweet with a floral aroma.', 'premium',  1],
      ['Kesar',            '🟡', 349, 120, 'Gir, Gujarat',           'Saffron-hued flesh, honey-sweet with a distinctive fragrance. Best for aamras.',  'seasonal', 1],
      ['Chaunsa',          '🥭', 429, 45,  'Punjab',                 'Silky, nectar-sweet with an intoxicating scent. Highly prized variety.',            'premium',  1],
      ['Dasheri',          '🟠', 279, 60,  'Lucknow, UP',            'Long, slim mangoes with smooth, sweet pulp. Perfect for summer drinks.',            'seasonal', 0],
      ['Langra',           '🍋', 259, 90,  'Varanasi, UP',           'Green even when ripe, with a tangy-sweet, slightly tart flavour profile.',          'bulk',     0],
      ['Totapuri',         '🫑', 199, 200, 'Andhra Pradesh',         'Large, elongated with mild tartness. Great for pickles and juices.',                'bulk',     0],
    ];
...
app.post('/api/products', async (req, res) => {
  ...
  try {
    const result = await run(
      'INSERT INTO products (name,emoji,price,stock,origin,description,type,featured) VALUES (?,?,?,?,?,?,?,?)',
      [name, emoji || '🥭', price, stock || 0, origin || '', description || '', type || 'seasonal', featured ? 1 : 0]
    );
...
app.post('/api/orders', async (req, res) => {
  ...
  try {
    ...
    for (const item of items) {
      ...
      if (product.stock < item.qty) return res.status(400).json({ error: `Not enough stock for ${item.name} (only ${product.stock}kg left)` });
    ...
    const result = await run(
      ...
      [customer_name, phone, address, city || '', pincode || '', payment || 'upi', JSON.stringify(items), subtotal, delivery || 0, total]
    );
